...
def inception_v3(inputs, 
    ...
    with tf.variable_scope(scope, 'InceptionV3', [network_in],
        ...
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            epsilon = hyper_param[0]
            alpha_c = hyper_param[1]
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], 
                ...
                pr5 = slim.conv2d(concat5, 2*(time_step-1), [3, 3], activation_fn=None, scope='pr5')
                ...
                pr4 = slim.conv2d(concat4, 2*(time_step-1), [3, 3], activation_fn=None, scope='pr4')
                ...
                scale = 2
                pr3 = slim.conv2d(concat3, 2*(time_step-1), [3, 3], activation_fn=None, scope='pr3')
                ...
                pr2 = slim.conv2d(concat2, 2*(time_step-1), [3, 3], activation_fn=None, scope='pr2')
                ...
                pr1 = slim.conv2d(concat1, 2*(time_step-1), [3, 3], activation_fn=None, scope='pr1')
...
def loss_interp_multi(flows, inputs, outputs, epsilon, alpha_c, alpha_s, lambda_smooth, flow_scale, deltaWeights):
    ...
    for b in xrange(num_batch):
        ...
        for c in xrange(channels):
            ...
            x = floor_flows[b, :, idx1]    # U, horizontal displacement
            y = floor_flows[b, :, idx2]    # V, vertical displacement
            xw = weights_flows[b, :, idx1]
            yw = weights_flows[b, :, idx2]
    ...
    # gradientsMask = tf.sqrt(tf.pow(img_delta[:,:,:,0], 2) + tf.pow(img_delta[:,:,:,1], 2))

    Charbonnier_reconstruct = 0.0
    numValidPixels = 0.0
    if needMask:
        eleWiseLoss = tf.mul(borderMask_flat, eleWiseLoss)
        # gradientsMask = tf.tile(tf.expand_dims(gradientsMask, 3), [1, 1, 1, channels])
        # gradientsMask = tf.reshape(gradientsMask, [num_batch, -1, channels])
        # eleWiseLoss = tf.mul(tf.exp(-1.0 * tf.abs(gradientsMask)), eleWiseLoss)
    ...
    if needMask:
        ...
        eleWiseULoss = tf.pow(tf.square(flow_delta_clean[:,:,:,0::2]) + tf.square(epsilon), alpha_s)
        U_loss = tf.reduce_sum(eleWiseULoss) / numValidPixels
        eleWiseVLoss = tf.pow(tf.square(flow_delta_clean[:,:,:,1::2]) + tf.square(epsilon), alpha_s)
...
def VGG16(inputs, outputs, loss_weight, hyper_param, is_training=True):
